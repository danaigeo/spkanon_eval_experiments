...
def filter_speakers_with_few_utterances(max_speakers, scenario, seed, all_utterances, min_utterances_per_speaker, file_name, stage):
    """
    ...
    eligible_speakers = [speaker_id for speaker_id, utterances in all_utterances.items() if len(utterances) >= min_utterances_per_speaker]
    
    if len(eligible_speakers) > max_speakers:
        selected_speakers = random.sample(eligible_speakers, max_speakers)
    else:
...
def filter_out_enroll_utterances(max_enrolls, f_trials, original_enrolls, seed, scenario, stage):
    """
    Selects a random set of utterances for each speaker from a dictionary of utterances,
    excluding any speaker with fewer utterances than the specified number per speaker.
    ...
    # Select max_enrolls utterances per speaker, ensuring they are not in used_utterances
    selected_utterances = []
    for speaker_id, utterances in utterances_per_speaker.items():
        ...
        if len(available_utterances) >= max_enrolls:
            selected_utterances.extend(random.sample(available_utterances, max_enrolls))
        else:
            selected_utterances.extend(random.sample(utterances, min(max_enrolls, len(utterances))))
...
def sample_and_filter_speakers(args, write_file, min_utter, filtered_utterances_by_speaker, iterations_per_group, original_enrolls, root_folder, seed = None):
    """
    ...
    num_utterances_per_speaker = args.max_trials
    ...
    for iteration in range(iterations_per_group):
        ...
        # Fitler out max_enroll random enroll utterances that are not included trials
        f_enrolls = filter_out_enroll_utterances(args.max_enrolls, f_trials, original_enrolls, seed, scenario, "trial")
        ...
        print(f"No. of speaker pairs: {len(pairs)}")
        ...
        #ASV Config
        print(f"Iteration: {iteration}, Unique pairs {len(unique_pairs)}, {thresholds[key]}, {eer}")
        ...
        with open(write_file, "a") as f:
            f.write(f"{seed} {args.file} {args.max_speakers} {num_utterances_per_speaker} {args.max_enrolls} {len(unique_pairs)} {len(pairs)} {thresholds[key]} {eer} {end-start}\n")
...
def main(args):
    ...
    max_speakers = args.max_speakers  # Define the number of speakers to be evaluated
    ...
    filtered_eval, num_speakers = filter_speakers_with_few_utterances(max_speakers, scenario, seed, utterances_by_speaker, min_utter, f_eval, "eval")
    print(f"No. of speakers: {num_speakers}, No. of trials: {args.max_trials}, No. of enrolls: {args.max_enrolls}")
    
    filtered_utterances_by_speaker = load_utterances(filtered_eval)

    # Perform privacy evaluation
    sample_and_filter_speakers(args, write_file, min_utter, filtered_utterances_by_speaker, iterations_per_group, f_enrolls, root_folder, seed)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_speakers", type=int, help="#Speakers", default=50)
    parser.add_argument("--max_trials", type=int, help="#Trial utterances per Speaker", default=5)
    parser.add_argument("--max_enrolls", type=int, help="#Enrollment utterances per Speaker", default=20)
    parser.add_argument("--min_utter", type=int, help="Minimum utterances per Speaker", default=120)
